...
//char bluetooth_device_addr[18] = {0};

RemoteDeviceInfo remote_dev_i;

/* device name */
static const char local_device_name[] ="Ocean";
...
static void bt_app_gap_cb(esp_bt_gap_cb_event_t event, esp_bt_gap_cb_param_t *param)
{
    ...
    case ESP_BT_GAP_AUTH_CMPL_EVT: {
        if (param->auth_cmpl.stat == ESP_BT_STATUS_SUCCESS) {
			...
			strcpy(remote_dev_i.remote_device_name, (char *)param->auth_cmpl.device_name);
        } else {
			printf("Auth Failed");
...
void host_stack(){
	//Set default config
	 ...
	 esp_err_t bluedroid_status = esp_bluedroid_init_with_cfg(&bluedroid_config);
	 //printf("%s",esp_err_to_name(bluedroid_status));
	 //Initialize
	 esp_err_t bluedroid_init_stat = esp_bluedroid_init_with_cfg(&bluedroid_config);
	 //printf("%s", esp_err_to_name(bluedroid_init_stat));
	 //Enable
	 esp_err_t bluedroid_enable_stat = esp_bluedroid_enable();
	 //printf("%s", esp_err_to_name( bluedroid_enable_stat));
